  ...
  return (
    <div className="mx-auto p-4 w-full max-w-screen-xl bg-gradient-to-r from-[#04071D] via-[#04071D] to-[#0C0E23] border border-[rgba(105,113,162,0.16)] shadow-lg backdrop-blur-md transition-colors duration-300 rounded-lg text-slate-300">
      ...
      {systems.length > 0 ? (
              ...
              >
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                    <h2 className="text-xl font-semibold">Информация о системе: {system.system}</h2>
                  ...
                  {/* Средняя колонка: аномалии */}
                  <div className="flex-1 space-y-2">
                    <h3 className="text-lg font-semibold text-slate-400">Доступные Аномалии</h3>
                    {anomalies ? (
                      <div className="space-y-4">
                              ...
                              <div className="pl-6 space-y-1">
                                {(entries || []).map((anomaly: Anomaly, idx: number) => (
                                  <div key={idx} className="flex items-center gap-2">
                                    <span className="text-green-400 font-medium text-sm">{anomaly.name}</span>
                                    <span className="text-red-500 font-bold text-sm">{anomaly.value}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                  ...
                  <div className="flex-1 space-y-2">
                    <h3 className="text-lg font-semibold text-slate-400">Триггеры следующей волны</h3>
                    {anomalies ? (
                      <Accordion type="single" collapsible className="w-full">
                        {(Object.values(anomalies).flat() as Anomaly[]).map((anomaly, idx) => {
                          const triggers = getTriggersForAnomaly(anomaly.name);
                          if (!triggers) return null;

                          return (
                            <AccordionItem key={idx} value={`item-${idx}`}>
                              <AccordionTrigger className="text-green-400 font-medium text-sm">
                                {triggers.Name}
                              </AccordionTrigger>
                              <AccordionContent className="pl-6 space-y-2">
                                {Object.entries(triggers.Triggers).map(([wave, trigger]) => (
                                  <div key={wave} className="flex items-center gap-2">
                                    <span className="text-slate-400 text-xs">{wave.toUpperCase()}</span>
                                    <Image
                                    ...
                                    <span className="text-slate-400 text-xs">({trigger.Distance.trim()})</span>
                                  </div>
                                ))}
                              </AccordionContent>
                            </AccordionItem>
                      ...
                      <p className="text-slate-500">Нет данных о триггерах</p>
                    )}
                  </div>
                </div>
              </div>
        ...
        <p className="text-center text-slate-300">Введите название wormhole: J100744</p>
      )}
    </div>
  );
}
...
//                   {/* Правая колонка: триггеры */}
//                   <div className="flex-1 space-y-2">
//                     <h3 className="text-lg font-semibold text-slate-400">Триггеры</h3>
//                     {anomalies ? (
//                       <div className="space-y-4">
//                         {(Object.values(anomalies).flat() as Anomaly[]).map((anomaly, idx) => {
//                           const triggers = getTriggersForAnomaly(anomaly.name);
...
//                             <div key={idx}>
//                               <div className="mb-2">
//                                 <span className="text-green-400 font-medium text-sm">{triggers.Name}</span>
//                               </div>
//                               <div className="pl-6 space-y-1">
//                                 {Object.entries(triggers.Triggers).map(([wave, trigger]) => (
//                                   <div key={wave} className="flex items-center gap-2">
